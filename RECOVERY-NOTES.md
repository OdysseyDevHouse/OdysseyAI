...
| ~~invoice product picker~~ | **RESTORED 2026-08-09** by the session that originally wrote it, so this is the real thing rather than a guess: the "Add product" search dialog on `(app)/sales/invoicing/[id]`, its department filter and 500-product auto-load, `listProductDepartmentsAction`, and `browseForTill`'s `term` parameter (added alongside the offline-catalog rewrite above, not in place of it). Verified end to end in a browser. |
| ~~salesperson default~~ | **RESTORED 2026-08-09** — `repsForLines` in `lib/site/lookups.ts` plus `defaultRepUserId` on the invoice/quote editor, so a new line is attributed to whoever is capturing. |
| ~~`IDLE` in `attachmentActions.ts`~~ | **RESTORED 2026-08-09** — it had been re-exported from a `'use server'` file, which fails at runtime with "can only export async functions, found object" and took **every** action on the invoicing page down with it. Moved back into `AttachmentsPanel`. |
| ~~three hydration fixes~~ | **RESTORED 2026-08-09** — `toLocaleDateString(undefined, …)` in `TimesheetScreen`, `PayRulesScreen` and `ClockScreen` given an explicit `'en-ZA'`. The two remaining `(pos)` instances belong to the offline work and were left alone. |
