...
import { STATUS_LABELS, STATUS_TONE } from '@/app/(app)/sales/status'
 ...
 * (/invoicing/[id]) because there is still work to do on it, while a
...
function hrefFor(doc: InvoiceTableRow): string {
  return isOpen(doc) ? `/invoicing/${doc.id}` : `/sales/${doc.id}`
}
