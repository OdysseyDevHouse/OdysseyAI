...
import {
  deleteStorefrontImageAction,
  listStorefrontImagesAction,
  uploadStorefrontImageAction,
} from './actions'

/**
 * Choosing the picture on a banner.
 *
...
function PickerDialog({
  ...
  useEffect(() => {
    ...
    setLoading(true)
    ;(async () => {
      const list = await listStorefrontImagesAction()
  ...
  function upload(file: File) {
    startAction(async () => {
      ...
      form.set('file', file)
      const result = await uploadStorefrontImageAction(form)
      if (!result.ok) {
  ...
  function remove(image: StorefrontImage) {
    startAction(async () => {
      const result = await deleteStorefrontImageAction(image.id)
      if (!result.ok) {
        toast.error(result.error)
        return
      }
