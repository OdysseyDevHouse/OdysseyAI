...
export function DataTable<T>({
  ...
  const allSelected =
    selectableRows.length > 0 && selectedHere === selectableRows.length;
        ...
        <table className={TABLE}>
                  ...
                  >
                    {column.sortable ? (
                      ...
                      >
                        {column.header}
                        <SortGlyph
                          direction={sorted ? activeSort.direction : undefined}
                        />
                      </button>
                    ) : (
                      column.header
          ...
          <tbody>
            {sortedRows.map((row) => {
              ...
              const rowSelectable =
                selectable && (isRowSelectable?.(row) ?? true);
